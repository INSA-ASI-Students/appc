...
% et         0 <= x_i  <= C_i
%
% Crï¿½dits : J.P. Yvon (INSA de Rennes) pour l'algorithme
...
while STOP ~= 1
    ...
    % nouvele resolution du systï¿½me
...
%     end
        ...
        M = At*(U\(U'\Ae));
        d = U\(U'\ce);
        d = (At*d - be);   % second membre  (homage au gars OlgaZZZZZZ qui nous a bien aidï¿½)
        ...
        lambda = M\d;
        % On reconstitue le vecteur en rï¿½solvant le systï¿½me linï¿½aire Hx = c-Alambda
        % size(lambda)
        % size(Ae)
        %      keyboard
    ...
    if (sum(xnew< 0) > 0 | sum(xnew > C(ind)) > 0)   &  length(ind) > ncc  %  03/01/2002 AR
        ...
        % semi param
        % on projette dans le domaine admissible et on sature la contrainte associï¿½e
        %-------------------------------------------------------------------------
        ...
        mu = H*xt - c + A*lambda;    % calcul des multiplicateurs de lagrange associï¿½es aux contraintes

        indsat = 1:n;                           % on ne regarde que les contraintes saturï¿½es
